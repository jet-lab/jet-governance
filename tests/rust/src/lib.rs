...
mod tests {
    ...
    use crate::helper::{now, TestClient, TestRealm};
    ...
    fn only_proposal_owner_may_edit() -> Result<()> {
        ...
        let proposer = realm.init_voter(&realm_owner)?;
        let proposal =
            proposer.init_proposal("draft", "this is a draft", Time::Never, Time::Never)?;
    ...
    #[test]
    fn voter_should_not_be_able_to_incrementally_withdraw_more_tokens_than_they_deposited(
    ) -> Result<()> {
        let client = TestClient::new();
        let realm = TestRealm::new(&client)?;
